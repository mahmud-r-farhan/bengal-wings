...
bengal-wings/
├── playground/
│   ├── 01_flight_telemetry_stream.go       # [Go] High-Performance Telemetry Stream Engine
│   ├── 02_pid_flight_controller.cpp        # [C++] Real-Time PID Flight Control Loop
│   ├── 03_yolo_object_detection.py         # [Python] Computer Vision & Object Tracking
│   ├── 04_drone_state_machine.rs           # [Rust] Memory-Safe Flight State Machine
│   ├── 05_mavlink_parser_bench.c           # [C] Bare-Metal Fast MAVLink Protocol Parser
│   ├── 06_mavlink_telemetry_parser.py      # [Python] Binary MAVLink Telemetry Processing
...
| --- | --- | --- | --- |
| **Core Flight Logic** | `01` - `05` | Go, C++, Python, Rust, C | High-frequency PID loop, telemetry streams, YOLO vision tracking, state machine. |
...
```bash
python3 playground/03_yolo_object_detection.py
python3 playground/06_mavlink_telemetry_parser.py
python3 playground/18_wifi_tcp_socket_server.py

```

### 2. C / C++ Files (GCC / Clang)

```bash
# Compile and run bare-metal driver
gcc -O2 playground/17_pcb_i2c_sensor_driver.c -o i2c_driver
./i2c_driver

# Compile LoRa driver
gcc -O2 playground/21_lora_sx1276_spi_driver.c -o lora_driver
./lora_driver
...
```bash
go run playground/01_flight_telemetry_stream.go
...
```bash
chmod +x playground/10_auto_system_diagnostics.sh
./playground/10_auto_system_diagnostics.sh

chmod +x playground/24_system_setup_and_deploy.sh
./playground/24_system_setup_and_deploy.sh
...
> 💡 **Tip:** Use the root **`Makefile`** shortcut commands (e.g., `make setup`, `make build`, `make run`) to automatically compile and test key modules.
